...
export class FormContactComponent implements OnInit {
  ...
  constructor(private translate: TranslateService, private genCaptcha: EmailService, private domSanitizer: DomSanitizer, private sendMail: EmailService, private sendMailCusco: EmailService) {

    
    this.crearFormulario()
    this.procesarConsulta()
    this.procesarConsultaCusco()
  }
...
CorreoEmisor: new FormControl('',[Validators.required, Validators.pattern("^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$")], ),
  ...
  enviarCorreoCusco(hotelSeleccionado) {
    ...
    cuerpo = this.datos.value
        ...
        if (this.datos.valid) {
            ...
            this.sendMailCusco.enviarCorreoCusco(cuerpo).subscribe(
                (data) => { 
                    console.log('Correo Enviado a Cusco') 
  ...
  enviarCorreo(hotelSeleccionado) {
    let cuerpo = new EmailData();
    cuerpo = this.datos.value

    // if (hotelSeleccionado == 'hotel Cusco'){
        if (this.datos.valid) {
    ...
    // if (hotelSeleccionado == 'Hotel Mapi'){
    //     if (this.datos.valid) {
    ...
    // generar captcha hotel Cusco
    procesarConsultaCusco() {
        this.showLoadingSpinner = true;
        this.genCaptcha.obtenerCaptchaCusco().subscribe(
        data => {
            this.capcha64Cusco = this.domSanitizer.bypassSecurityTrustResourceUrl("data:image/bmd;base64, " + data );
        },
        error => {
            this.showErrorConexionMessage();
        },
